/**     @file ModelPartList.h
  ...
  *
  *     P Evans 2022
  *     @brief Defines the ModelPartList class, a tree model structure for managing ModelPart objects in a Qt TreeView.
  *
  * This class inherits from QAbstractItemModel and implements all required methods to represent a hierarchical
  * structure of model parts (such as 3D STL objects) in a Qt item view. It supports dynamic addition, removal,
  * and editing of parts, and acts as the data source for the application’s tree-based UI.
  *     edited by Zhixing REN 2025
  */
 ...
 *
 * Cleans up the root item, which in turn deletes all its children.
 */
    ~ModelPartList();
...
/**
 * @brief Appends a child item with the given data under the specified parent.
 *
 * Automatically emits signals to notify Qt of model changes.
 *
 * @param parent The QModelIndex under which the new child is inserted.
 * @param data List of QVariant values representing the child's columns.
 * @return QModelIndex of the newly inserted child.
 */
    QModelIndex appendChild( QModelIndex& parent, const QList<QVariant>& data );
/**
 * @brief Sets the data for a given index, if the value is valid.
 *
 * Trims whitespace and ignores empty values to prevent invalid entries.
 *
 * @param index The QModelIndex to modify.
 * @param value The new value to set.
 * @param role Typically Qt::EditRole.
 * @return True if the data was successfully updated; false otherwise.
 */
    bool setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole);

/**
 * @brief Removes the ModelPart at the specified index from the tree.
 *
 * Deletes the item and its children, and emits signals to update the view.
 *
